...
function configurePlanner(type) {
  ...
  $("#planDescription").textContent = quarter ? "Confira o recorte e selecione as habilidades. O trimestre será organizado por eixos, sem divisão em semanas: o calendário não ganhou poderes de adivinhação." : "Defina os encontros, as habilidades e os recursos disponíveis. O texto será organizado aula a aula; conhecer quem aprende continua sendo trabalho humano.";
  ...
  $(".summary-help").textContent = quarter ? "O documento será organizado em eixos do trimestre e contemplará as habilidades oficiais selecionadas." : "O documento será criado aula a aula, respeitando aulas-faixa e a fé nas escolhas pedagógicas marcadas acima.";
...
function buildPromptBase() {
  ...
  const format = getMeetingFormat();
  const lessons = isQuarter ? `aproximadamente ${item.lessons * 12} aulas, articuladas em três eixos formativos do trimestre` : `${format.total} aulas organizadas em ${format.simple} aula(s) simples de ${format.duration} minutos${format.bandMeetings ? ` e ${describeBands(format)}` : ""}`;
  ...
  const structureInstruction = isQuarter
    ? "Organize em três eixos temáticos e progressivos do trimestre. Para cada eixo, apresente foco, conteúdos, habilidades, propostas de aprendizagem e evidências de acompanhamento. Não organize por semanas nem em sequência rígida de aulas."
...
function buildPlan() {
  ...
  $("#sequenceTitle").textContent = isQuarter ? "Eixos do trimestre" : "Sequência de encontros";
